...
class NotifyComingVC(commands.Cog):
    ...
    def __get_user_name(self, member):
        return member.name if member.nick is None else member.nick

    async def __notify_afk(self, member, before, after):
        name = self.__get_user_name(member)
    ...
    async def __notify_connected(self, member, before, after):
        name = self.__get_user_name(member)
    ...
    async def on_voice_state_update(self, member, before, after):
        await self.__notify_afk(member, before, after)
        await self.__notify_connected(member, before, after)
